...
import { TrendingUp, TrendingDown, Users, CreditCard, Target, Award } from "lucide-react";
...
export default function EnhancedStatsCards({ data, trends }: EnhancedStatsCardsProps) {
  const getTrendData = (key: string) => {
    const trendData = trends[key] || [];
    if (trendData.length === 0) return null;
    
    const latest = trendData[trendData.length - 1];
    const chartData = trendData.slice(-10).map(item => ({ value: item.value }));
  ...
  const statsData: StatsCardData[] = [
    {
      ...
      value: data.accountsVerified.toLocaleString(),
      icon: Users,
      color: "text-valasys-orange",
      bgColor: "bg-orange-50",
      trend: getTrendData('accountsVerified'),
      target: { current: data.accountsVerified, target: 1500 },
    },
    {
      ...
      target: { current: data.availableCredits, target: 20000 },
    },
    {
      title: "Credits Spent",
      value: data.creditsSpent.toLocaleString(),
      icon: Target,
    ...
    },
    {
      ...
      trend: getTrendData('successRate'),
      target: { current: data.successRate, target: 95 },
    },
  ];
              ...
              {stat.target && (
                <div className="space-y-2">
                  ...
                  <div className="text-xs text-valasys-gray-500">
                    Target: {stat.target.target.toLocaleString()}
                  </div>
                </div>
              )}
            </CardContent>
